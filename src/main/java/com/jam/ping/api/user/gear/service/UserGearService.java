package com.jam.ping.api.user.gear.service;
...
public class UserGearService {
    ...
    public UserGearDto createUserGear(Long categoryId, Long makerId, Long gearId, String name, String memo) {
        ...
        if (gear == null && (resolvedName == null || resolvedName.isBlank())) {
            throw new BadRequestException("기어를 선택하지 않은 경우 장비명은 필수입니다.");
        }
    ...
    private UserGear findUserGear(Long userGearId) {
        return userGearRepository.findById(userGearId)
                .orElseThrow(() -> new NotFoundException("장비 기록을 찾을 수 없습니다."));
    }
    ...
    private void checkOwnership(UserGear userGear) {
        if (!userGear.getUser().getId().equals(AuthUtils.getCurrentUserId())) {
            throw new ForbiddenException("본인의 장비 기록만 수정/삭제할 수 있습니다.");
        }
    }
}
